...
select*
from IPLPlayers;
# find the total spending on players fro each team
select Team,sum(Price_in_cr) as total_spending
from IPLPlayers
group by Team
order by total_spending;
# 2 find the top 3 highets paid all rounders across all team 
select DISTINCT Player,Team,Price_in_cr
from IPLPlayers
where Role='ALL-rounder'
order by Price_in_cr DESC
limit 3;
# HIGHEST PRKICED PALYER IN EACH TEAM
SELECT *
from IPLPlayers
...
select I.Player,I.team,m.max_price
from IPLPlayers I join cte_mp m
on I.Team=m.Team;
# rank player by their price in each team anf list the top 2 for every team
with rnk_table as
 (select Player,Team,Price_in_cr,
Row_Number() over(partition by Team order by Price_in_cr DESC ) as rnk
from IPLPlayers)
select Player,Team,Price_in_cr,rnk
from rnk_table 
where rnk<=2;

# find dthe most expensive player from each team ,along with the second most expensive players  name and price 
with rnk_table as
 (select Player,Team,Price_in_cr,
Row_Number() over(partition by Team order by Price_in_cr DESC ) as rnk
from IPLPlayers)
select Team,
MAX(CASE WHEN  rnk=1 then Player END) as mostexpensiveplayer,
max(case when rnk=1 then Price_in_cr end) as highestprice,
max(case when rnk=2 then Player end) as secondmostexpensiveplayer,
max(case when rnk=2 then Price_in_cr end) as secondhighestprice
from rnk_table
group by Team;
# calculate percentage contribution of ecah players price to their teams total spending
select Team,Price_in_cr,Player,
CAST(
    (Price_in_cr/sum(Price_in_cr) over(partition by Team))*100.0
    as decimal(10,2)
) as contributionprice
from IPLPlayers;

#high medium ,low prices based on the following rules high :price>15 medium price b/w 5 and 15 cr and low price <5 cr and find the number of players in each bracket
with cte_bar as (select Team,Player,Price_in_cr,
case
...
# find the avg price of indian players and comapre it with overseas players using a subquery


select 
'Indian' as playertypes,
 avg(Price_in_cr)
from IPLPlayers
...
GROUP BY playertype;
# identify the olayer who earn more than avg price in their team
select Price_in_cr,Team,Player
from IPLPlayers p1
where Price_in_cr>
(select avg(Price_in_cr) from IPLPlayers p2 where p1.Team=p2.Team);
# for each role find moste expensive player and their price using a correlated query
select Price_in_cr,Player,Role
from IPLPlayers p1
where Price_in_cr=(select max(Price_in_cr) from IPLPlayers p2 where p1.Role=P2.Role); (server issue n*n time run query)


SELECT Price_in_cr, Player, Role
FROM (
    SELECT *,
           MAX(Price_in_cr) OVER (PARTITION BY Role) AS max_price
    FROM IPLPlayers
) t
WHERE Price_in_cr = max_price;